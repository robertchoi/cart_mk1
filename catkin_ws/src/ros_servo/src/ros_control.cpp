...
const float ANGLE_TO_TURN_BACK = 4.0f;
...
void cart_judge(float x, float y)
{
	...
	if(x >= 0){
		...
		(x < X_CLOSE && -ANGLE_TO_TURN < angle && angle < ANGLE_TO_TURN) ? S_STOP : 
		(X_CLOSE < x && x < X_FAR && angle > ANGLE_TO_TURN) ? S_LEFT : 
		(X_CLOSE < x && x < X_FAR && angle < -ANGLE_TO_TURN) ? S_RIGHT : 
		(x > X_FAR && angle > ANGLE_TO_TURN) ? S_LEFT : 
		(x > X_FAR && angle < -ANGLE_TO_TURN) ? S_RIGHT : 
		(x < X_CLOSE && angle < -ANGLE_TO_TURN) ? S_RIGHT : 
		(x < X_CLOSE && angle > ANGLE_TO_TURN) ? S_LEFT : 
		...
		(X_BACK_CLOSE < x && -ANGLE_TO_TURN_BACK < angle && angle < ANGLE_TO_TURN_BACK) ? S_STOP : 
		(x < X_BACK_CLOSE && X_BACK_FAR < x && angle > ANGLE_TO_TURN_BACK) ? S_LEFT : 
		(x < X_BACK_CLOSE && X_BACK_FAR < x && -ANGLE_TO_TURN_BACK < angle) ? S_RIGHT : 
		(x < X_BACK_FAR && angle > ANGLE_TO_TURN_BACK) ? S_LEFT : 
		(x < X_BACK_FAR && angle < -ANGLE_TO_TURN_BACK) ? S_RIGHT : 
		(x > X_BACK_CLOSE && angle < -ANGLE_TO_TURN) ? S_RIGHT : 
		(x > X_BACK_CLOSE && angle > ANGLE_TO_TURN) ? S_LEFT : 
