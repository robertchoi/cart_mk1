...
sensor_msgs::PointCloud2 cloud_cb(const pcl::PCLPointCloud2 &input)
{
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it, j++)
    {
        ...
        for (std::vector<int>::const_iterator pit = it->indices.begin(); pit != it->indices.end(); ++pit)
        {
            ...
            pt3.x = pt2.x = -pt.x, pt3.y = pt2.y = -pt.y, pt3.z = pt2.z = pt.z;
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    ros::Subscriber sub = nh.subscribe("/lidar0/scan", 10, scan_cb);
    ...
    pub = private_nh.advertise<sensor_msgs::PointCloud2>("/rp_cluster", 5);
    cluster_point_pub = nh.advertise<sos_fusion::MinMax> ("/target_object_position", 5);
    cluster_array_pub = nh.advertise<sos_fusion::PointArray> ("/objects_positions", 5);
