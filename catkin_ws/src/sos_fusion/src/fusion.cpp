...
int main(int argc, char** argv){
    ...
    ros::Subscriber person_sub = nh.subscribe("/target_object_position", 10, person_cb);
    ros::Subscriber button_sub = nh.subscribe("/button", 10, button_cb);

    line_pub = nh.advertise<visualization_msgs::Marker>("/line_pub",10);
    text_pub = nh.advertise<visualization_msgs::Marker>("/text_pub",10);
    bbox_pub = nh.advertise<visualization_msgs::Marker>("/bbox_pub",10);
